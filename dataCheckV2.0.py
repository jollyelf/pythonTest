...
import os
import arcpy
import dataCheckFunctionToFile
...
def main(workSpace):
    ...
    for FC in featureClassList:
        dataCheckFunctionToFile.pipeSegmentLogicTest(FC)    #检查管段数据中的各种逻辑错误
        dataCheckFunctionToFile.fieldIsNonetest(FC)         #检查必填字段是否为空
        dataCheckFunctionToFile.codeRepetitionTest(FC)      #检查编码是否重复
        dataCheckFunctionToFile.facilityAffiliationTest(FC) #检查设备设施与管段编码是否匹配
        dataCheckFunctionToFile.facilityCodeTest(FC)        #检查设备设施的编码是否符合编码规则
        dataCheckFunctionToFile.fieldIsSameTest(FC)         #检查设备设施的某些字段是否与管段相同
        dataCheckFunctionToFile.facilityDiameterTest(FC)    #检查设备设施的直径是否与管段匹配     
