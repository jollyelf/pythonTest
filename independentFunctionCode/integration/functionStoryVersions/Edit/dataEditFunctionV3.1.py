...
def fieldsFill(feature):
    ...
    DiameterDNDic={"14.0":10,"15.0":10,"17.0":10,"17.2":10,"18.0":15,"20.0":15,"21.3":15,"22.0":15,"25.0":20,"26.9":20,\
                   ...
                   "76.1":65,"88.9":80,"89.0":80,"90.0":80,"104.0":100,"108.0":100,"110.0":100,"114.0":100,"114.3":100,\
                ...
                with arcpy.da.UpdateCursor(feature,("PSCODE",FGFT)) as cursor:
                    for row in cursor:
                        try:
                            ...
                            print featureClassCodeDictionary[feature],"编辑属性时出错，错误信息：",e.message
                            pass
                        continue
                    ...
                    for row in cursor:
                        try:
                            if row[0] is not None:
                                for PLS in PipeSegmentDataList:
                                    if FDT!="OUTDIAMETER":
                                        #if row[0]==PLS[1] and PLS[fieldsSequenceDic["DIAMETER"]] is not None and row[1] is None:
                            ...
                            print featureClassCodeDictionary[feature],"编辑管径时出错，错误信息：",e.message
                            pass
                        continue
                ...
                with arcpy.da.UpdateCursor(feature,("PSCODE",FTT)) as cursor:
                    for row in cursor:
                        try:
                            ...
                            print featureClassCodeDictionary[feature],"编辑壁厚时出错，错误信息：",e.message
                            pass
                        continue
#定义删除重复要素函数
            ...
            for row in cursor:
                try:
                    if row[0] is not None and len(row[0])==14 and re.search(r'\d{6}GB\d{6}',row[0]):
                        ...
                        #如果管段名称中包含高速，穿越公路等级=高速公路，其他的穿越公路等级=街道
...
def editThreeorFour():
    '''
    在正确的管段归属划分基础上，编辑三通四通表。
    因空间连接的不准确的原因，单独将三通表的属性编辑制作成一个独立的函数
    要运行此函数，三通表必须已完成管段编码字段填写
    基本思路：
    首先获取管段数据信息
    然后依据管段信息，已管段编码作为连接字段，将管段信息按照既定规则来编辑三通表的主管径、主管壁厚等其他属性；
    同时也可以依据经验和三通表自身的数据分析来填写相关字段，如连接方式，坐标信息，以及类型等。
    '''
    #设定直径与DN值之间的健-值关系
    DiameterDNDic={"14.0":10,"15.0":10,"17.0":10,"17.2":10,"18.0":15,"20.0":15,"21.3":15,"22.0":15,"25.0":20,"26.9":20,\
                   "32.0":25,"33.7":25,"34.0":25,"38.0":32,"40.0":32,"42":32,"42.4":32,"45.0":40,"48.0":40,\
    ...
    with arcpy.da.SearchCursor("T_PN_PIPESEGMENT_GEO",("CODE","DIAMETER","THICKNESS","USEDDATE",\
                                                       "CONSTRUNIT","SUPERVISORUNIT","TESTUNIT","FDNAME","COLLECTDATE",\
                                                       "COLLECTUNIT","INPUTDATETIME","DESIGNPRESURE","NAME")) as PPcursor:
        for PC in PPcursor:
            if PC[0] is not None and str(PC[0]).strip()!="":
                PipeSegmentDataList.append([PC[0],PC[1],PC[2],PC[3],PC[4],PC[5],PC[6],PC[7],PC[8],PC[9],PC[10],PC[11],PC[12]])
    #更新三通数据
    tempDictionary={}       
...
def editDefaultValuesForFeature():
    '''
    为部分要素表的特定字段填写默认值
    包括：阀井阀室、阀门、钢塑转换接头、调压箱、异径管、聚缘接头和弯头，主要涉及压力等级，连接方式和防腐相关属性
    基本思路：
    首先获取管段相关数据
    然后依据管段信息填写部分字段（比如压力等级、防腐材料等），以及依据经验设置合适的默认值（比如出入口连接方式等）
    '''
    #获取管段数据
    PipeSegmentDataList=[]
        ...
        with arcpy.da.UpdateCursor("T_PN_VALVEPIT_GEO",("PSCODE","DESIGNPRESSURE","RUNPRESSURE")) as VPcursor:
            for VProw in VPcursor:
                try:
                    ...
                    pass
                continue
    #编辑阀门表，压力等级=管段设计压力，阀门构造类型=球阀，驱动类型=手动，进口/出口连接类型=焊接，正常/当前阀门状态=常开，阀门功能=调节，是否可远程控制=否
        ...
        with arcpy.da.UpdateCursor("T_PN_VALVE_GEO",("PSCODE","PRESSURELEVEL","VALVETYPE","DRIVETYPE","INCONNECTMODE",\
            ...
            for Vrow in Vcursor:
                try:
                    ...
                    pass
                continue
    #编辑刚塑转换接头表，压力等级=管段设计压力，进口/出口连接方式=焊接
    if int(arcpy.GetCount_management("T_PN_SPE_GEO").getOutput(0))!=0:
        with arcpy.da.UpdateCursor("T_PN_SPE_GEO",("PSCODE","PRESSURELEVEL","INCONNECTMODE","OUTCONNECTMODE")) as SPcursor:
            for SProw in SPcursor:
                try:
                    for PSD in PipeSegmentDataList:
                        if SProw[0]==PSD[0]:
                            SProw[1]=PSD[1]
                    if SProw[0] is not None:
                        SProw[2]=1
                        SProw[3]=1
        ...
        with arcpy.da.UpdateCursor("T_PN_REGULATOR_GEO",("PSCODE","INPRESSURE","RUNPRESSURE")) as REcursor:
            for RErow in REcursor:
                try:
                    ...
                    pass
                continue
    #编辑异径管表，压力等级=管段设计压力，进口/出口连接方式=焊接
        ...
        with arcpy.da.UpdateCursor("T_PN_IJOINT_GEO",("PSCODE","PRESSURELEVEL","INCONNECTMODE","OUTCONNECTMODE")) as IJcursor:
            for IJrow in IJcursor:
                try:
                    ...
                    pass
                continue
    #编辑弯头表，压力等级=管段设计压力，弯管类型=正常弯头，入口/出口连接方式=焊接,材料=碳钢（如果管段材料不是PE）,防腐材料=3PE(管段为3PE)=石油沥青（管段为石油沥青）
            ...
            for ELrow in ELcursor:
                try:
                    for PSD in PipeSegmentDataList:
                        if ELrow[0]==PSD[0]:
                            ELrow[1]=PSD[1]
                            if PSD[3] is not None and PSD[3]!=22 and PSD[3]!=23 and PSD[3]!=24 and PSD[3]!=25:
                                ELrow[5]=1
                            if PSD[4] is not None:
                                if PSD[4]==1:
                                    ELrow[6]=14
                                elif PSD[4]==3:
                                    ELrow[6]=1
